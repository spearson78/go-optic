...
Lenses to access the fields of a struct are usually auto-generated using the [makelens](/docs/9.makelens) tool

{{< playground file="/content/docs/2.concepts/examples_data_test.go" id="optics_lens_makelens" >}}

The [makelens](/docs/9.makelens) tool generated the `data.O.BlogPost().Content()` optic providing access to the `BlogPost.content` field.
...
For this use case of delving into a nested structure this compose syntax is verbose. It is advised to use the [makelens](/docs/9.makelens) tool to generate helpers that make this use case much more intuitive. The above example can be simplified to the following when using [makelens](/docs/9.makelens)
...
The `ComposeLeft` has retained the `[int]` index of the `TraverseSlice`. As a convenience the `Optics` generated by [makelens](/docs/9.makelens) use `ComposeLeft` when the right side of the `Compose` is un-indexed.
